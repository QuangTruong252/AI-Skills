...
def validate_skill(errors: list[str]) -> None:
    ...
    require("name: figma-ui" not in text, "SKILL.md must not use the legacy figma-ui name", errors)
    require("always stop after" not in text.lower(), "SKILL.md must not require unconditional mapping review", errors)
...
def main() -> int:
    ...
    if errors:
        print("figma-to-ui package validation: FAIL")
        for error in errors:
            print(f"- {error}")
        return 1

    print("figma-to-ui package validation: PASS")

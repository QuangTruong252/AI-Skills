...
result = ROOT / "templates" / "feature-change-result.md"
if result.is_file():
    ...
    for marker in (
        ...
        if marker not in text:
            errors.append(f"templates/feature-change-result.md: missing marker: {marker}")
    for legacy in ("workflow_scope_result:", "recommended_task_state:"):
        if legacy in text:
            errors.append(f"templates/feature-change-result.md: legacy contract {legacy}")
...
if errors:
    print("feature-change package structure: FAIL")
    for error in errors:
        print(f"- {error}")
    sys.exit(1)

print("feature-change package structure: PASS")
